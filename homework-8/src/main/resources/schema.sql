...
(
    ...
    references ukhinms.products
(
    id
) on delete cascade,
    id_basket integer not null
    references ukhinms.baskets
(
    id
)
  on delete cascade,
...
CREATE
OR REPLACE FUNCTION ukhinms.update_basket_price()
    RETURNS TRIGGER
AS
'
BEGIN
...
CREATE
OR REPLACE TRIGGER products_baskets_insert_trigger
    AFTER
UPDATE
